...
Mesh createPlaneMesh(float width, float depth, int divisions)
{
    Mesh plane;
    ...
    for (int z = 0; z <= divisions; ++z)
    {
        ...
        {
            float xPos = -halfWidth + (width * x) / divisions;
            float zPos = -halfDepth + (depth * z) / divisions;
            float u    = static_cast<float>(x) / divisions;
            float v    = static_cast<float>(z) / divisions;

            plane.vertices.push_back({glm::vec3(xPos, 0.0f, zPos), glm::vec3(0.0f, 1.0f, 0.0f), glm::vec2(u, v)});
        }
    }
    ...
    for (int z = 0; z < divisions; ++z)
    {
        for (int x = 0; x < divisions; ++x)
        {
            ...
            plane.triangles.emplace_back(topLeft, bottomLeft, topRight);
            plane.triangles.emplace_back(topRight, bottomLeft, bottomRight);
        }
    }

    // Simple gray material
    plane.material.kd = glm::vec3(0.8f, 0.8f, 0.8f);
